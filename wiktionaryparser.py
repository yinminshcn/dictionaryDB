...
class WiktionaryParser(object):
    def __init__(self):
        self.url = "https://en.wanweicidian.com/wiki-{}?printable=yes"
        self.soup = None
    ...
    def parse_pronunciations(self, word_contents):
        ...
        for pronunciation_index, pronunciation_id, _ in pronunciation_id_list:
            tags = self.soup.find_all('span', {'id': pronunciation_id})
            if len(tags) == 0:
                continue
            span_tag = tags[0]
    ...
    def parse_definitions(self, word_contents):
        ...
        for def_index, def_id, def_type in definition_id_list:
            definition_text = []
            tags = self.soup.find_all('span', {'id': def_id})
            if len(tags) == 0:
                continue
            span_tag = tags[0]
    ...
    def fetch(self, word, language=None):
        ...
        try:
            response = self.session.get(self.url.format(word), timeout=20)
